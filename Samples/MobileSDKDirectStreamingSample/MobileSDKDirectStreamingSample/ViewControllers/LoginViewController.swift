...
class LoginViewController: UIViewController {
    ...
    @IBAction func login(_ sender: Any) {
        ...
        serverInfo.isSecureConnection = isHTTPS()
        ...
        XPMobileSDK.connectAndLoginForServerInfo(serverInfo,
            ...
            self?.alert(withTitle: Constants.Login.loginErrorTitle,
                        message: error?.localizedDescription ?? Constants.Login.loginErrorMessage)
        })
    }
    ...
    @IBAction func onConnectionTypeChanged(_ sender: Any) {
        port.text = isHTTPS() ? Constants.Login.loginDefaultSecurePort : Constants.Login.loginDefaultPort
    }
    ...
    private func alert(withTitle title: String, message: String) {
        ...
        let defaultAction = UIAlertAction(title: Constants.alertOK,
                                          style: .default,
                                          handler: nil)
    ...
    private func isHTTPS() -> Bool {
        return connectionType.selectedSegmentIndex > 0
    }
}
...
extension LoginViewController: XPMobileSDKConnectionDelegate {
    func connectionDidFailConnectWithError(_ error: NSError) {
        alert(withTitle: Constants.Login.loginErrorTitle,
